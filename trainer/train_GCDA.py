...
def train(args, cfg, model_instance, optimizer_g, optimizer_f):
    ...
    for i in trange(cfg.max_iter):
        ...
        classifier_loss, contrastive_loss, min_entroy_loss, max_entroy_loss  = model_instance.get_loss(inputs, labels_source)
...
if __name__ == '__main__':
    from model.network_GCDA import model
    ...
    # init model
    if cfg.dataset == 'office-home':
        symmetric_graph = np.load("../data/officehome/symmetric_graph_office_home.npy")
        node_num = 87
    model_instance = model(cfg, use_gpu=True, symmetric_graph=symmetric_graph, node_num=node_num)
